...
public class ShellService {
     ...
     * @return true 启动了
     */
    public boolean getMasterStatus(String roomId) {
        String masterProcessNum = this.getMasterProcessNum(roomId);
     ...
     * @return true 启动了
     */
    public boolean getCavesStatus(String roomId) {
        String cavesProcessNum = this.getCavesProcessNum(roomId);
...
//    }
     ...
     * @return 执行信息
     */
    public List<String> startMaster(String roomId) {
        //开始游戏是安装mod
     ...
     * 优雅关闭地面服务
     */
    public void elegantShutdownMaster(String roomId) {
        this.shutdownMaster(roomId);
        if (this.getMasterStatus(roomId)) {
            //运行中 睡眠
            for (int i = 0; this.getMasterStatus(roomId); i++) {
     ...
     * 优雅关闭洞穴服务
     */
    public void elegantShutdownCaves(String roomId) {
        this.shutdownCaves(roomId);
        if (this.getCavesStatus(roomId)) {
            //运行中 睡眠
            for (int i = 0; this.getCavesStatus(roomId); i++) {
                if (i == MAX_SLEEP_SECOND) {
                    break;
                } else {
                    this.sleep(1);
     ...
     * @return 执行信息
     */
    public List<String> updateGame(String roomId) {
        //优雅关闭
    ...
    public ResultVO<String> installModToServer(String roomId) {
        ...
        //待修改问题 mod会越来越多
        //获取原先已有的mod列表，与单个room的mod排重后重新覆盖写入全部mod列表
        List<String> allModList = ModFileUtil.readAllModConfigFile(serverModPath);
        allModList = allModList.stream().filter(x -> !modConfig.contains(x)).collect(Collectors.toList());
        modConfig.addAll(allModList);
